...
class Tensor:
    ...
    def matmul(self, other):
        ...
        if len(self.shape) < 2 or len(other.shape) < 2:
            raise ValueError(
                f"Matrix multiplication requires at least 2D tensors\n"
                f" Got shapes: {self.shape} @ {other.shape}\n"
                f" Use * for element-wise multiplication of scalars or 1D tensors\n"
                f" 🔧 Reshape to 2D: tensor.reshape((1, -1)) or tensor.reshape((-1, 1))"
            )
