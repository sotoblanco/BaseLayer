...
def main():
    ...
    parser.add_argument(
        "--lesson-path", required=True, help="Path to lesson directory to write metadata.json"
    )
    parser.add_argument("--title", default=None, help="Optional title for the copied sheet")
    parser.add_argument(
        "--make-public",
        action="store_true",
        help="Make the copied sheet editable by anyone with the link",
    )
    ...
    if not sa_file:
        print(
            "ERROR: Set GOOGLE_SERVICE_ACCOUNT_FILE or SERVICE_ACCOUNT_FILE to your service account JSON key"
        )
        return
    ...
    try:
        ...
        print(
            "ERROR: Missing google client libs. Install: pip install google-api-python-client google-auth"
        )
        print(e)
        return
    ...
    if args.make_public:
        try:
            drive.permissions().create(
                fileId=new_id, body={"type": "anyone", "role": "writer"}
            ).execute()
    ...
    metadata = {"exercise_type": "spreadsheet", "google_sheet_id": new_id, "copy_on_open": True}
